...
describe("Password Reset", () => {
  ...
  describe("createResetToken", () => {
    ...
    it("should create different tokens for different emails", async () => {
      const token1 = await createResetToken("test1@example.com");
      const token2 = await createResetToken("test2@example.com");
      
      expect(token1).not.toBe(token2);
    });

    it("should replace existing token for same email", async () => {
      const token1 = await createResetToken("test@example.com");
      const token2 = await createResetToken("test@example.com");
      
      // Should create a new token (replacing the old one)
      expect(token2).toBeDefined();
      // The old token should be invalid
      const result1 = await verifyResetToken(token1);
  ...
  describe("verifyResetToken", () => {
    ...
    it("should return invalid for non-existent token", async () => {
      const result = await verifyResetToken("nonexistent-token");
    ...
    it("should return invalid for already used token", async () => {
      ...
      const result1 = await verifyResetToken(token);
      expect(result1.valid).toBe(true);
      
      // Second verification should fail (token already used)
      const result2 = await verifyResetToken(token);
